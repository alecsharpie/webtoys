...
class TestAIService:
    ...
    async def test_generate_code_success(self, ai_service, mock_claude_response):
        ...
        with patch('services.ai_service.httpx.AsyncClient') as mock_client:
            ...
            result = await ai_service.generate_code(description="Create a simple canvas animation")
            ...
            called_args = mock_post.call_args[0][0]
            called_json = mock_post.call_args[1]["json"]
            
            assert "messages" in called_json
            assert called_json["messages"][0]["role"] == "user"
            assert "Create a simple canvas animation" in called_json["messages"][0]["content"]
    ...
    async def test_generate_code_api_error(self, ai_service):
        ...
        with patch('services.ai_service.httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(Exception) as exc_info:
                await ai_service.generate_code(description="Create a simple canvas animation")
                
            assert "API error" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_generate_code_with_parameters(self, ai_service, mock_claude_response):
        ...
        with patch('services.ai_service.httpx.AsyncClient') as mock_client:
            ...
            result = await ai_service.generate_code(
                description="Create a complex space animation",
