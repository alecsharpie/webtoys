...
class StorageService:
    ...
    def _init_file_storage(self):
        ...
        self.base_dir = Path(self.connection_string) if self.connection_string else Path("./storage")
        ...
        asyncio.create_task(self._cleanup_old_previews())
    ...
    def _init_s3_storage(self):
        ...
        try:
            ...
            raise RuntimeError(f"S3 storage initialization failed: {str(e)}")
    ...
    def _init_database_storage(self):
        ...
        try:
            ...
            raise RuntimeError(f"Database storage initialization failed: {str(e)}")
    
    async def store_preview(self, preview_id: str, code: Dict[str, str], metadata: Dict[str, Any]) -> str:
        """
    ...
    async def publish_webtoy(self, webtoy_id: str, code: Dict[str, str], metadata: Dict[str, Any]) -> str:
        """
    ...
    async def get_webtoy(self, webtoy_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def list_webtoys(self, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        """
    ...
    async def _store_preview_file(self, preview_id: str, code: Dict[str, str], metadata: Dict[str, Any]):
    ...
    async def _get_preview_file(self, preview_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _publish_webtoy_file(self, webtoy_id: str, code: Dict[str, str], metadata: Dict[str, Any]):
    ...
    async def _get_webtoy_file(self, webtoy_id: str) -> Optional[Dict[str, Any]]:
        ...
        for file_path in paginated_files:
            try:
                with open(file_path, 'r') as f:
                    ...
                    webtoys.append({
                        "id": webtoy_id,
                        "description": webtoy_data["metadata"].get("description", ""),
                        "created_at": webtoy_data["metadata"].get("created_at", 0)
                    })
            except (json.JSONDecodeError, IOError) as e:
                logger.error(f"Failed to load WebToy file {file_path}: {str(e)}")
            ...
            try:
                ...
                for file_path in preview_files:
                    try:
                        ...
                        logger.error(f"Failed to process file {file_path} during cleanup: {str(e)}")
                ...
                logger.error(f"Error in preview cleanup task: {str(e)}")
    
    # S3 storage implementation (simplified)
    async def _store_preview_s3(self, preview_id: str, code: Dict[str, str], metadata: Dict[str, Any]):
        """Store preview in S3"""
        preview_data = {
            "code": code,
            "metadata": metadata
        ...
        self.s3.put_object(
            Bucket=self.bucket,
            Key=f"previews/{preview_id}.json",
            Body=preview_json
        )
    
    async def _get_preview_s3(self, preview_id: str) -> Optional[Dict[str, Any]]:
        """Get preview from S3"""
        try:
            ...
            logger.error(f"Failed to get preview from S3: {str(e)}")
            return None
    
    async def _publish_webtoy_s3(self, webtoy_id: str, code: Dict[str, str], metadata: Dict[str, Any]):
        ...
        self.s3.put_object(
            Bucket=self.bucket,
            Key=f"webtoys/{webtoy_id}.json",
            Body=webtoy_json
        )
    
    async def _get_webtoy_s3(self, webtoy_id: str) -> Optional[Dict[str, Any]]:
        """Get WebToy from S3"""
        try:
            response = self.s3.get_object(
                Bucket=self.bucket,
            ...
            logger.error(f"Failed to get WebToy from S3: {str(e)}")
            return None
        ...
        try:
            ...
            for item in contents:
                try:
                    ...
                    logger.error(f"Failed to process S3 object {item['Key']}: {str(e)}")
            
            return webtoys
            
        except Exception as e:
            logger.error(f"Failed to list WebToys from S3: {str(e)}")
            return []
    
    # Database storage implementation (simplified)
    async def _store_preview_db(self, preview_id: str, code: Dict[str, str], metadata: Dict[str, Any]):
    ...
    async def _get_preview_db(self, preview_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _publish_webtoy_db(self, webtoy_id: str, code: Dict[str, str], metadata: Dict[str, Any]):
    ...
    async def _get_webtoy_db(self, webtoy_id: str) -> Optional[Dict[str, Any]]:
